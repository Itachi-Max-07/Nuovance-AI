      ...
      <div className="relative mt-5 h-64 sm:h-72">
        ...
        >
          {PATHS.map((path) => (
            <g key={path}>
              ...
              <path d={path} className="ws-dash stroke-brand-accent-2/70" strokeWidth="2" fill="none" />
            </g>
          ))}
        </svg>
        ...
        <div className="absolute left-0 top-1/2 hidden -translate-y-1/2 sm:block">
          <div className="ws-item">
            <div className="ws-glass flex h-16 w-16 items-center justify-center rounded-full text-brand-accent-2">
          ...
          {launch.targets.map((target, index) => {
            ...
            return (
              <div key={target} className="ws-item" style={{ animationDelay: `${index * 150}ms` }}>
                <div className="ws-glass flex items-center gap-2.5 px-4 py-3">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-accent/15 text-brand-accent-2">
                    <Buildings size={14} aria-hidden="true" />
                  </span>
                  <p className="min-w-0 flex-1 truncate text-xs font-medium text-brand-offwhite">
                    {target}
                  </p>
                  <span
                    className={cn(
                      ...
                      isDelivered && "border-brand-accent/40 text-brand-accent-2",
                      !isSent && !isDelivered && "border-brand-line text-brand-muted",
                    )}
                  >
                    {label}
                  </span>
